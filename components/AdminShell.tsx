...
const ADMIN_LOGO = '/brand/thynkxp-logo.png';
...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  function goDashboard(view: DashboardView) {
    if (pathname === '/admin') { activateLegacyDashboardSection(view); return; }
  ...
  const groups: { label: string; items: NavItem[] }[] = [
    ...
    { label: 'Equipe', items: [{ label: 'Gerenciar', icon: 'users', href: '/admin/equipe' }] },
    { label: 'Clientes', items: [{ label: 'Gerenciar', icon: 'briefcase', href: '/admin/clientes' }] },
    { label: 'Prospecção', items: [{ label: 'Visão Geral', icon: 'trending-up', dashboardView: 'opportunities' }] },
  ];
  ...
  return (
    <div className="admin-shell-v4">
      ...
      <aside className={`admin-v4-sidebar ${sidebarOpen ? 'is-open' : ''}`}>
        ...
        <div className="admin-v4-workspace"><span>TX</span><div><small>PAINEL ADMIN</small><strong>ThynkXP</strong><em>Inteligência comercial</em></div></div>
        <nav className="admin-v4-nav" aria-label="Navegação administrativa">
          {groups.map((group) => <section key={group.label} className="admin-v4-nav-group"><h3>{group.label}</h3><div>{group.items.map((item) => item.href ? (
          ...
          <section className="admin-v4-nav-group admin-v4-leads-group">
            <h3>Leads</h3>
            <button type="button" className={`admin-v4-dropdown-trigger ${pathname === '/admin/leads' || pathname === '/admin/radar' ? 'is-active' : ''}`} onClick={() => setLeadsOpen((open) => !open)} aria-expanded={leadsOpen}><Icon name="workflow" size={17} /><span>LEADS</span><Icon name="chevron-right" size={14} /></button>
            <div className={`admin-v4-subnav ${leadsOpen ? 'is-open' : ''}`}><a href="/admin/leads" className={pathname === '/admin/leads' ? 'is-active' : ''}><span>Gerenciar</span></a><a href="/admin/radar" className={pathname === '/admin/radar' ? 'is-active' : ''}><span>Buscar Leads</span></a></div>
          </section>
        </nav>
